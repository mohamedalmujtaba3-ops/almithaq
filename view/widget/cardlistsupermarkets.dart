...
class CardSupermarketsList extends GetView<SupermarketsController> {
  
  final SellersModel listdata;
  final String? search;
  final String? city;
  const CardSupermarketsList(this.search, this.city, {Key? key, required this.listdata}) : super(key: key);
  ...
  Widget build(BuildContext context) {
      ...
      child: ListTile(
        onTap: () {
            if(listdata.sellerStatus == "1") {
              ...
              Get.snackbar("عفواً", "المتجر قد أغلق الطلبات للوقت الحالي.");
            }
        },
        leading: Padding(
    ...
    // if there is a clinic that match search text
    ...
    // if there are no records within user city
    : const SizedBox(height: 0.1,);
