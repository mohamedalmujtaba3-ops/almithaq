...
class GmapPickLocation extends StatefulWidget {
  const GmapPickLocation({super.key});

  @override
  State<GmapPickLocation> createState() => _ModernLocationPickerState();
}

class _ModernLocationPickerState extends State<GmapPickLocation> {
  ...
  final List sellersCoord = Get.arguments['sellersCoord'];
  ...
  Future<List<Map<String, dynamic>>> _searchPlaces(String query) async {
    ...
    final url = Uri.https(
      ...
      '/maps/api/place/autocomplete/json',
      {
        ...
        'radius': '50000',
        'strictbounds': 'true', // THIS is the key to blocking outside results
        'key': kGoogleApiKey,
        'language': 'ar', // Optional: forces Arabic results (remove if not needed)
      },
    );
                      ...
                      CustomButtomAuth( text: "تم التحديد",
                        onPressed: () {
                            ...
                            double long = _selectedLocation!.longitude;
                              Get.toNamed(AppRoute.orderform, arguments: {
                                "lat": lat,
                                "long": long,
